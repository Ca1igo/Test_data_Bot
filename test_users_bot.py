...
TOKEN = '6280292414:AAGMhYVSxP-EiUklie0-bb_Z6H_TlOcb_LE'
...
def start_message_handler(message: types.Message):
    ...
    bot.send_message(
        ...
        text="Привет!\nЭто бот генерирует тестовых пользователей. "\
...
def message_handler(message: types.Message):
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=f"Данные для {payload_len} тестовых пользователей:\n<code>"\
        f"{payload_str}</code>"
    )
    bot.send_message(
        chat_id=message.chat.id,
        text="Если нужно сгенерировать данные еще раз жми 👇🏻",
        reply_markup=main_menu_reply_markup
    )
